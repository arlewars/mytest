...
class OIDCDebugger:
    ...
    def setup_ui(self): 
        ...
        self.response_text = tk.Text(self.frame, height=10, width=70)
        ...
        # Add new log text frame
        self.log_text = tk.Text(self.frame, height=10, width=70)
        self.log_text.grid(row=17, column=0, columnspan=2, padx=5, pady=5, sticky="ew")
        log_text_scrollbar = ttk.Scrollbar(self.frame, orient="vertical", command=self.log_text.yview)
        self.log_text.configure(yscrollcommand=log_text_scrollbar.set)
        log_text_scrollbar.grid(row=17, column=2, sticky="ns")

        self.certificate_btn = ttk.Button(self.frame, text="Show Certificate", command=self.show_certificate)
        self.certificate_btn.grid(row=18, column=1, padx=2, pady=5)

        self.replace_certificate_btn = ttk.Button(self.frame, text="Replace Certificate", command=self.replace_certificate)
        self.replace_certificate_btn.grid(row=18, column=0, padx=2, pady=5)
    ...
    def fetch_well_known(self):
        ...
        try:
            ...
            self.write_to_response_text(f"Well-known configuration loaded.\n")
            if self.log_oidc_process.get():
    ...
    def start_https_server(self):
        ...
        print(f"new: HTTPS server started on https://{server_name}:{self.server_port}/callback")
        try:
            https_server_thread.start()
            print(f"new2: HTTPS server start")
    ...
    def exchange_code_for_tokens(self, code, aud):
        self.log_message(f"Exchanging code for tokens: {code}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.schedule_ui_update( f"Token Exchange Response: {tokens}\n")
                self.add_horizontal_rule()
    ...
    def display_tokens(self, tokens):
        self.log_message(f"Displaying tokens: {tokens}")
    ...
    def decode_jwt(self, token):
        self.log_message(f"Decoding JWT: {token}")
    ...
    def userinfo_query(self, token, token_type):
        self.log_message(f"Userinfo query: {token_type} is {token}")
    ...
    def introspect_token(self, token, token_type):
        self.log_message(f"Introspecting token: {token_type} is {token}")
        try:
            data = {
                "token": token,
